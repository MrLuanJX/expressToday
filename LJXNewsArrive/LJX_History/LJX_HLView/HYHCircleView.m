...
- (void)createSubview{
    ...
    [self.titleLab mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo (weakSelf.bgView.mas_top).offset(weakSelf.bgView.height/5);
        make.height.mas_equalTo (20);
    }];
    
    [self.progressLab mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo (weakSelf.bgView.mas_centerX);
    ...
    [self.pointView mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo ((lineWH - weakSelf.pointView.width)/2);
    }];
...
- (void)createLayerPath{
    ...
    self.bgLayer.lineWidth = 10;//线条宽度
    self.bgLayer.strokeColor = kRGBColor(0xF6F6F9).CGColor;//线条颜色
    ...
    self.leftGradientLayer.frame = CGRectMake(0, 0, lineWH/2, lineWH);
    // 0x191970  0x4682B4
    [self.leftGradientLayer setColors:[NSArray arrayWithObjects:(id)kRGBColor(0x8DEEEE).CGColor,(id)kRGBColor(0x5CACEE).CGColor,(id)kRGBColor(0x4682B4).CGColor,  nil]];
    [self.leftGradientLayer setLocations:@[@0.0,@1.0]];
    [self.leftGradientLayer setStartPoint:CGPointMake(0, 0)];
    [self.leftGradientLayer setEndPoint:CGPointMake(0, 1)];
    [gradientLayer addSublayer:self.leftGradientLayer];
    
    self.rightGradientLayer = [CAGradientLayer layer];
    self.rightGradientLayer.frame = CGRectMake(lineWH/2, 0, lineWH/2, lineWH);
    // 0xEBD6AB   0xC6A05D
    [self.rightGradientLayer setColors:[NSArray arrayWithObjects:(id)kRGBColor(0x8DEEEE).CGColor,(id)kRGBColor(0x5CACEE).CGColor,(id)kRGBColor(0x4682B4).CGColor,  nil]];
    [self.rightGradientLayer setLocations:@[@0.0,@1.0]];
    [self.rightGradientLayer setStartPoint:CGPointMake(0, 0)];
    [self.rightGradientLayer setEndPoint:CGPointMake(0, 1)];
...
- (void) setProgressTitle : (NSString *) progressTitle Font:(UIFont *)font Color:(UIColor *)color {
    ...
    self.titleLab.text = progressTitle;
}
// 进度
- (void) setProgress:(CGFloat)progress {
    _progress = progress;
    
    [self didClickStartBtnAction];
}
// 进度条宽度
- (void) setProgressLineWidth:(CGFloat)width NormalLineWidth:(CGFloat)normallineWidth {
    self.progressLayer.lineWidth = width;
    self.bgLayer.lineWidth = normallineWidth;
}
// 渐变色
- (void) setGradientColorArray:(NSArray *)colorArray {
    
    self.leftGradientLayer.colors = colorArray;
    self.rightGradientLayer.colors = colorArray;
}

#pragma mark - action

- (void)didClickStartBtnAction{
...
- (UILabel *)titleLab{
    if (_titleLab == nil) {
        ...
        _titleLab.textColor = kRGBColor(0x8B0000);//kRGBColor(0xCCCCCC);
        _titleLab.text = @"已完成";
